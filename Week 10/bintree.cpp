...
const bintree_node *bintree::find(int value) const {
...
bool bintree::remove(int value) {
    if (m_root == nullptr) {
        ...
        if (node != nullptr) {
            ...
            // rebalance the tree bottom-up, starting with the removed node's parent
            rebalance(removed->m_parent);
...
void bintree::rebalance(bintree_node *bottom_node) {
    while (bottom_node != m_sentinel) {
        bottom_node->update_height();
        bottom_node->rebalance();

        // update height again
        bottom_node->update_height();

        bottom_node = bottom_node->m_parent;
    }
    update_root();
}
